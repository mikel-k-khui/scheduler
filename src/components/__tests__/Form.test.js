...
import { render, cleanup, fireEvent, queryByText, 
  getByAltText, getByText, getByPlaceholderText } from "@testing-library/react";
...
describe("Form", () => {
  ...
  const { container } = render(<Form student={""} interviewers={interviewers} onSave={mockSave} />);
  ...
  it("calls onSave function when the name is defined", () => {
    const mockSave = jest.fn();
    const mockArray =[];
    const { container } = render(<Form student={students.name} interviewers={interviewers}  onSave={mockSave} />);
    ...
    expect(mockSave).toHaveBeenCalledWith("Lydia Miller-Jones", interviewers[0]);
  });
